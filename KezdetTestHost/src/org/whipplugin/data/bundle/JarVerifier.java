...
package org.whipplugin.data.bundle;
...
public class JarVerifier
{
  ...
  public static void verify( JarFile jf, X509Certificate[] trustedCaCerts ) throws IOException, CertificateException, SecurityException
  {
    ...
    while( e.hasMoreElements() )
    {
      ...
      if( ( certs == null ) || ( certs.length == 0 ) )
      {
        if( !je.getName().startsWith( "META-INF" ) )
          throw new SecurityException( "Unsigned files detected" );
      }
      else
      {
  ...
  public static boolean isTrusted( X509Certificate cert, X509Certificate[] trustedCaCerts )
  {
    ...
    for( int i = 0; i < trustedCaCerts.length; i++ )
    {
      ...
      Principal trustedDN = trustedCaCerts[i].getSubjectDN();
